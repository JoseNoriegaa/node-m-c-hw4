...
/**
 * This function is used to configure a directory of statistical files
 * @param {String} rootDir is the path that contains the all static files
 * @param {String} endPoint is the base route on the server that return the static files
 */
server.static = (rootDir, endPoint) => {
  ...
  endPoint = typeof endPoint === 'string' && endPoint.length > 0
    ? endPoint : false;
  // check up if the parameters are valid
  if (rootDir && endPoint) {
    // check up if the "endpoint" contains a slash at the end, if it contains it, it will be removed
    endPoint = endPoint[endPoint.length - 1] === '/' ? endPoint.substring(0, endPoint.lastIndexOf('/')) : endPoint;
    // get the path of each file within the directory
...
const routesHandler = (req, res) => {
  ...
  req.on('end', () => {
    ...
    let contentType = headers['content-type'] ? headers['content-type'].trim().toLowerCase() : {};
    ...
    // get the server path inside the router, if it exist
    const routeHandler = getRoute(trimmedPath, method);
    // get static route, if it exists
    ...
    // Check up if the request is looking for static content
