...
cli.responders.product = async (str) => {
  let cmd = str;
  // Check up if the command has a parameter
  cmd = cmd.split(' --');
  if (cmd.length > 1) {
...
cli.responders.help = () => {
  ...
  const commands = {
    ...
    'clear': 'It clears the screen or window of the command console',
    'product, OPTIONS: --all, --one {product name} :': 'Show info about available products',
    'user, OPTIONS: --all, --one {email} :': 'Show info about the registed users',
    'last sessions': 'View all the users who have signed up in the last 24 hours.',
